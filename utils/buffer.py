...
class ReplayBuffer(object):
    ...
    def __init__(self, max_steps, num_agents, obs_dims, ac_dims):
        ...
        for odim, adim in zip(obs_dims, ac_dims):
            ...
            self.penalty_buffs.append(np.zeros(max_steps, dtype=np.float32))
    ...
    def push(self, observations, actions, rewards, penalties, next_observations, dones):
        ...
        if self.curr_i + nentries > self.max_steps:
            ...
            for agent_i in range(self.num_agents):
                ...
                self.rew_buffs[agent_i] = np.roll(self.rew_buffs[agent_i],
                                                  rollover)
                self.penalty_buffs[agent_i] = np.roll(self.penalty_buffs[agent_i],
                                                  rollover)
        ...
        for agent_i in range(self.num_agents):
            ...
            self.penalty_buffs[agent_i][self.curr_i:self.curr_i + nentries] = penalties[:, agent_i]
    ...
    def sample(self, N, to_gpu=False, norm_rews=True):
        ...
        return ([cast(self.obs_buffs[i][inds]) for i in range(self.num_agents)],
                ...
                ret_rews,
                [cast(self.penalty_buffs[i][inds]) for i in range(self.num_agents)],
