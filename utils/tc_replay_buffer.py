...
class ReplayBuffer(object):
    ...
    def __init__(self, max_steps, num_agents, obs_dims, ac_dims):
        ...
        for odim, adim in zip(obs_dims, ac_dims):
            ...
            self.penalty1_buffs.append(np.zeros(max_steps, dtype=np.float32))
            self.penalty2_buffs.append(np.zeros(max_steps, dtype=np.float32))
    ...
    def push(self, observations, actions, rewards, penalties_1, penalties_2, next_observations, dones):
        ...
        if self.curr_i + nentries > self.max_steps:
            ...
            for agent_i in range(self.num_agents):
                ...
                self.rew_buffs[agent_i] = np.roll(self.rew_buffs[agent_i],
                                                  rollover)
                self.penalty1_buffs[agent_i] = np.roll(self.penalty1_buffs[agent_i],
                                                  rollover)
                self.penalty2_buffs[agent_i] = np.roll(self.penalty2_buffs[agent_i],
                                                       rollover)
        ...
        for agent_i in range(self.num_agents):
            ...
            self.penalty1_buffs[agent_i][self.curr_i:self.curr_i + nentries] = penalties_1[:, agent_i]
            self.penalty2_buffs[agent_i][self.curr_i:self.curr_i + nentries] = penalties_2[:, agent_i]
    ...
    def sample(self, N, to_gpu=False, norm_rews=True):
        ...
        return ([cast(self.obs_buffs[i][inds]) for i in range(self.num_agents)],
                ...
                ret_rews,
                [cast(self.penalty1_buffs[i][inds]) for i in range(self.num_agents)],
                [cast(self.penalty2_buffs[i][inds]) for i in range(self.num_agents)],
    ...
    def get_average_penalties(self, N):
        if self.filled_i == self.max_steps:
            ...
            inds = np.arange(max(0, self.curr_i - N), self.curr_i)
        return ([self.penalty1_buffs[i][inds].mean() for i in range(self.num_agents)],
                [self.penalty2_buffs[i][inds].mean() for i in range(self.num_agents)])
