...
class Scenario(BaseScenario):
    ...
    def reward(self, agent, world):  # Main Reward
    ...
    def global_reward(self, world):
        ...
        for d in self.deposits(world):
            ...
            if len(dists_to_holding) > 0:
                ...
                n_visible = 7
                # get positions of all entities in this agent's reference frame
                other_agent_inds = [a.i for a in world.agents if (a is not d and a.collector)]
                closest_agents = sorted(
                    zip(world.cached_dist_mag[other_agent_inds, d.i],
                        other_agent_inds))[:n_visible]
                closest_inds = list(i for _, i in closest_agents)
        ...
        for a in self.collectors(world):
            for b in self.collectors(world):
                if a != b:
                    if self.is_collision(a, b, world):
                        flag = 1

        for d in self.deposits(world):
            for dep in self.deposits(world):
                if d!= dep:
